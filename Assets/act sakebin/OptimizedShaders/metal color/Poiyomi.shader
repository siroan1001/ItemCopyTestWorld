Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/4402121275850bd4cb33c5d82a47f319"
{
	Properties
	{
 ...
 #define PROP_MATCAP2 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(0.399));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0.399), float(1));
				}
				else
				{
					finalWrap = max(finalWrap, float(0.399));
				}
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.068), float(0), float(0), float(0), float(0.85), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(0.399));
					poiLight.indirectColor = max(poiLight.indirectColor, float(0.399));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.399))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.399))));
				}
